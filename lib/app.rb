...
def stream
  ...
  login_streaming.filter(track: topics.join(",")) do |tweet|
...
#^^^ va chercher les >5< derniers tweet les plus >récents< avec le >'#bonjour_monde'<
# et mettre un j'aime 




#journalist.sample(6).each { |i| $client.update( "Bonjour #{i} viens follow @thehackingproject" ) }

#^^^ permet de tweeter avec 6 nom au piff 

#$client.update('Ceci est un tweet')

# ^^^ champ permettant de rentrer le twitte que l'on veut 
